...
(function() {
   ...
   */
    ...
    // ## Assign an issue to new assignee ##
    // ### Takes ###
    // *  issueId: Issue to assign
    // *  newAssignee: string of Jira user name
    // *  callback: for when it's done
    //
    // ### Returns ###
    // *  error string
    ...
    this.assignIssue = function (issueId, newAssignee, callback) {
        var options = {
            rejectUnauthorized: this.strictSSL,
        ...
        this.doRequest(options, function (error, response, body) {
            ...
            if (response.statusCode === 400) {
                callback('There is a problem with the received user representation.');
                return;
            } else if (response.statusCode === 401) {
                callback('The calling user does not have permission to assign the issue.')
                return;
            } else if (response.statusCode === 404) {
                callback('The issue or the user does not exist.')
                return;
            }

            callback(response.statusCode + ': Error while assigning issue');
        });
    };

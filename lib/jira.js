...
// [Jira Doc](http://docs.atlassian.com/jira/REST/latest/#id290791)
    ...
    // ### Takes ###
    // *  issueId: Issue to add a comment to
    // *  watcher: string containing watcher user name
    // *  callback: for when it's done
    //
    // ### Returns ###
    // *  error string
    // *  success string
    //
    // [Jira Doc](https://docs.atlassian.com/jira/REST/latest/#d2e2610)
    this.addWatcher = function(issueId, watcher, callback){
        var options = {
            rejectUnauthorized: this.strictSSL,
            uri: this.makeUri('/issue/' + issueId + '/watchers'),
            body: '"' + watcher + '"',
            method: 'POST',
            followAllRedirects: true,
            json: true
        ...
        this.doRequest(options, function(error, response, body) {
            ...
            if (response.statusCode === 400) {
                callback("Invalid user representation: " + JSON.stringify(body));
                return;
            }

            if (response.statusCode === 401) {
                callback("Invalid user permissions: " + JSON.stringify(body));
                return;
            }

            if (response.statusCode === 404) {
                callback("Issue " + issueId + " or user " + JSON.stringify(body) + " not found");
                return;
            }

            callback("Unknown status code returned: " + response.statusCode);
            return;
        });
    };

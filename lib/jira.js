...
var JiraApi = exports.JiraApi = function(protocol, host, port, username, password, apiVersion, verbose, strictSSL, oauth, context) {
	...
	//clean context so that it is an empty string or ends with /
    if((this.context) && (this.context.length > 0)) {
    	if(this.context.indexOf('/', this.context.length - 1) === -1) {
			this.context += '/';
    	}
    } else {
    	this.context = '';
    ...
    this.makeUri = function(pathname, altBase, altApiVersion) {
        ...
        var uri = url.format({
            ...
            hostname: this.host,
            port: this.port,
            pathname: this.context + basePath + apiVersion + pathname
        });

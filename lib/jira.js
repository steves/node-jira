...
(function() {
   ...
   */
    ...
    //
    // *  optional: object containing any of the following properties
    //   *  projectIds: (string) combined with the projectKeys param, lists the projects with which to filter the results
    //   *  projectKeys: (string) combined with the projectIds param, lists the projects with which to filter the results
    //   *  issuetypeIds: (string) combinded with issuetypeNames, lists the issue types with which to filter the results
    //   *  issuetypeNames: (string) combinded with issuetypeIds, lists the issue types with which to filter the results
    //   *  expandFields: (boolean) Default false. If true the response will include fields (adds expand=projects.issuetypes.fields)
    // *  callback: for when it's done
    //
    // ### Returns ###
    // *  error string
    // *  response object
    //
    // [Jira Doc](http://docs.atlassian.com/jira/REST/latest/#api/2/issue-getCreateIssueMeta)
    this.getCreateIssueMeta = function(optional, callback) {
        if (optional.expandFields) {
            optional.expand = 'projects.issuetypes.fields';
            delete optional.expandFields;
        }
        var params = [];
        for (i in optional) {
            if (!optional.hasOwnProperty(i)) continue;
            if (optional[i]) {
                params.push(encodeURIComponent(i) + '=' + encodeURIComponent(optional[i]))
            }
        }
        ...
        var options = {
            ...
            method: 'GET',
            body: {
              projectIds: optional.projectIds || '',
              projectKeys: optional.projectKeys || '',
              issuetypeIds: optional.issuetypeIds || '',
              issuetypeNames: optional.issuetypeNames || '',
        ...
        this.doRequest(options, function(error, response, body) {
            ...
            callback(response.statusCode + ': Error while retrieving create issue meta');

...
// [Jira Doc](http://docs.atlassian.com/jira/REST/latest/#id290791)
    ...
    // *  filter object
    //
    // [Jira Doc](https://docs.atlassian.com/jira/REST/latest/#d2e484)
    this.getFilter = function(filterId, callback) {
        var options = {
            rejectUnauthorized: this.strictSSL,
        ...
        this.doRequest(options, function(error, response, body) {
            ...
            if (response.statusCode === 404) {
                callback('Filter not found');
                return;
            }

            callback(response.statusCode + ': Error while getting filter', body);
    ...
    // ### Takes ###
    //
    // *  filter: an object of the new filter
    // *  callback: for when it's done
    //
    // ### Returns ###
    // *  error string
    // *  filter object
    //
    // [Jira Doc](https://docs.atlassian.com/jira/REST/latest/#d2e451)
    //
    /* {
     *     "name": "All Open Bugs",
     ...
     * }
     */
    this.createFilter = function(filter, callback) {
        var options = {
            rejectUnauthorized: this.strictSSL,
        ...
        this.doRequest(options, function(error, response, body) {
            ...
            callback(response.statusCode + ': Error while creating filter', body);
    ...
    // *  filter object
    //
    // [Jira Doc](https://docs.atlassian.com/jira/REST/latest/#d2e534)
    this.deleteFilter = function(filterId, callback) {
        ...
        this.doRequest(options, function(error, response, body) {
            ...
            callback(response.statusCode + ': Error while deleting filter', body);
    ...
    // *  filter object
    //
    // [Jira Doc](https://docs.atlassian.com/jira/REST/latest/#d2e505)
    //
    /* {
     *     "id": "10000", // Required
     *     "name": "All Open Bugs",
     ...
     * }
     */
    this.updateFilter = function(filter, callback) {
        var options = {
            rejectUnauthorized: this.strictSSL,
            uri: this.makeUri('/filter/' + filter.id),
            method: 'PUT',
            followAllRedirects: true,
            json: true,
            body: filter
        ...
        this.doRequest(options, function(error, response, body) {
            ...
            callback(response.statusCode + ': Error while updating filter', body);
